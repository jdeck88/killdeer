...
class Product {
  ...
  async updateInventory({ visible, track_inventory, stock_inventory }, accessToken) {
    const payload = { visible, track_inventory, set_inventory: stock_inventory };
    await axios.patch(`${utilities.LL_BASEURL}products/${this.productId}/`, payload, {
      headers: { Authorization: `Bearer ${accessToken}` }
    });
  }
  ...
  #calculatePrices() {
    ...
    return {
      ...
      productID: Number(this.data.localLineConnectedVendorProductID)
    };
  }
}
