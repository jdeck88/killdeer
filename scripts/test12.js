...
async function resolveCategoryName(variationId) {
  if (!variationId) return 'Unknown Variation';
...
(async () => {
  ...
  const netDeposit = totalCardReceived + standalonePaymentTotal - totalProcessingFees - totalRefunds;

  console.log(`📋 INVOICE REPORT: ${startArg} to ${endArg}\n`);
  console.log(`🛒 CATEGORY SALES:`);
  for (const [cat, amt] of Object.entries(categorySales)) {
    console.log(`  • ${cat}: $${(amt / 100).toFixed(2)}`);
  }
  console.log(`  --------------------------------------------------`);
  console.log(`  Subtotal (Category Sales): $${(totalSales / 100).toFixed(2)}`);

  console.log(`\n💵 RECEIVED:`);
  console.log(`  • Cash Received:     $${(totalCashReceived / 100).toFixed(2)}`);
  console.log(`  • Card Received:     $${(totalCardReceived / 100).toFixed(2)}`);
  if (standalonePaymentTotal > 0) {
    console.log(`  • Other Payments:    $${(standalonePaymentTotal / 100).toFixed(2)}`);
  }
